...
import static app.cosmos.reconbike.SMSReceive.id_get_is_auth_apply;
...
public class IdGetActivity extends AppCompatActivity {
    ...
    EditText phone;
    FrameLayout get_id_con, next_con;
    TextView get_id, auth_number_btn, count_text, next;
        ...
        public void run() {
            ...
            if (secCount == 0) {
                ...
                if (minCount < 0) {
                    ...
                    id_get_is_auth_apply = false;
                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "인증번호 입력시간이\n초과 되었습니다.\n다시 시도해 주세요.", "1", "확인");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_id_get);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                setXiaomiDarkMode(IdGetActivity.this);
                setMeizuDarkMode(IdGetActivity.this);
            }
        }
        ...
        get_id_con = (FrameLayout) findViewById(R.id.get_id_con);
        get_id = (TextView) findViewById(R.id.get_id);
        ...
        auth_number_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if ("".equals(phone.getText().toString())) {
                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "휴대폰 번호를\n입력해 주세요.", "1", "확인");
                    oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                    oneBtnDialog.setCancelable(false);
                    oneBtnDialog.show();
                    return;
                ...
                if (!checkPhoneNumber(phone.getText().toString().replaceAll("-", "").replaceAll("\\)", "").trim())) {
                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "올바른 휴대폰\n번호가 아닙니다.", "1", "확인");
                    oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                    oneBtnDialog.setCancelable(false);
                    oneBtnDialog.show();
                    return;
                    ...
                    public void callback(String url, JSONObject jsonObject, AjaxStatus status) {
                        try {
                            if ("true".equals(jsonObject.getString("return"))) {
                                Toast.makeText(IdGetActivity.this, "인증번호를 전송 하였습니다.", Toast.LENGTH_SHORT).show();
                                auth_number_btn.setEnabled(false);
                                count_text.setVisibility(View.VISIBLE);
                                count.run();
                                id_get_is_auth_apply = true;
                            } else if ("false".equals(jsonObject.getString("return"))) {
                                id_get_is_auth_apply = false;
                                if ("phone".equals(jsonObject.getString("type"))) {
                                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "가입되어 있지 않은\n전화번호 입니다.", "1", "확인");
                                    oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                    oneBtnDialog.setCancelable(false);
                                    oneBtnDialog.show();
                                } else {
                                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "SMS전송 서버 장애로 인해\n인증번호 문자를 발송 할 수\n없습니다.", "1", "확인");
                                    oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                    oneBtnDialog.setCancelable(false);
                                    oneBtnDialog.show();
                                }
        ...
        next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if ("".equals(auth_number.getText().toString()) || "" == auth_number.getText().toString()) {
                    oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "인증번호를 입력해 주세요.", "1", "확인");
                    oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                    oneBtnDialog.setCancelable(false);
                    oneBtnDialog.show();
                    return;
                ...
                aQuery.ajax(url, params, JSONObject.class, new AjaxCallback<JSONObject>() {
                    ...
                    public void callback(String url, JSONObject jsonObject, AjaxStatus status) {
                        try {
                            if ("true".equals(jsonObject.getString("return"))) {
                                get_id_con.setVisibility(View.VISIBLE);
                                get_id.setText(jsonObject.getString("id"));
                                phone.clearFocus();
                                ...
                                secCount = 60;
                                count_text.setVisibility(View.GONE);
                                oneBtnDialog = new OneBtnDialog(IdGetActivity.this, "인증번호가 틀렸습니다.\n다시 휴대폰 인증을 해주세요.", "1", "확인");
                                oneBtnDialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                oneBtnDialog.setCancelable(false);
                                oneBtnDialog.show();
                            }
